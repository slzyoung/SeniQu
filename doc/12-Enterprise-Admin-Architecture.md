...
This document details the comprehensive modernization of the SeniQu platform's administrative and operational infrastructure, focusing on UI/UX overhauls, security hardening, database schema stabilization, and terminology standardization applied in Version 1.1.0.
...
- **Super Admin Dashboard**: Full redesign of the centralized governance interface.
- **Museum Admin**: Curatorial management interface.
- **Gallery Admin**: Commercial exhibition management.
- **Heritage/Sites Admin**: Cultural site preservation tracking.
- **Artist Dashboard**: Personal portfolio and marketplace metrics.
...
## 3. Database Schema & RBAC Stabilization

Significant backend stability updates were executed to resolve complex data-fetching errors and improve the Role-Based Access Control (RBAC) architecture.

### 3.1 Role Scope Hardening (Migrations 044 & 045)
- Added `admin_scope_id` to the `users` table, creating a definitive hard-link between an institutional admin and their specific `institutions` record.
- Ensures strict Row-Level Security (RLS) implementation where Museum/Gallery admins can only access or modify records tied to their `admin_scope_id`.

### 3.2 Foreign Key Disambiguation (Resolving 500 Errors)
...
### 4.1 OTP-Based Password Changes
...
### 4.2 Resolving SQL Injection Guard False-Positives
...
## 5. Performance & Data Fetching (Anti-Chunking)
