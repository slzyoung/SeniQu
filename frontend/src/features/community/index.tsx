...
export function ThreadView() {
    ...
    const [localLikedForumComments, setLocalLikedForumComments] = useState<Record<string, boolean>>(() => {
        try {
            const saved = localStorage.getItem('liked_forum_comments');
    ...
    const [forumReplyingTo, setForumReplyingTo] = useState<{ id: string; name: string } | null>(null);
    const [expandedForumReplies, setExpandedForumReplies] = useState<Record<string, boolean>>({});
    const forumCommentInputRef = useRef<HTMLInputElement>(null);
    ...
    const handleReply = async () => {
        ...
        try {
            await createPost.mutateAsync({
                ...
                parentId: forumReplyingTo?.id || undefined
            });
            setReplyContent('');
                ...
                {showReplies && createPortal(
                    <>
                        ...
                        <div className="reel-drawer text-left bg-white dark:bg-[#121212] border-t border-gray-200 dark:border-white/10" style={{ zIndex: 99999 }}>
                            ...
                            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
                                {replies.length === 0 ? (
                                    ...
                                    topLevelReplies.map((reply: any) => {
                                        ...
                                        const isReplyLiked = !!localLikedForumComments[reply.id];
                                        const likes = isReplyLiked ? 1 : 0;
                                        const nestedReplies = childReplies.filter((r: any) => r.parent_id === reply.id || r.parentId === reply.id);

                                        return (
                                            <div key={reply.id} className="border-b border-gray-100 dark:border-white/5 pb-3 last:border-0 last:pb-0">
                                                {/* Main Comment */}
                                                <div className="flex gap-3 text-left items-start relative">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        <div className="flex items-center gap-3 mt-1.5">
                                                            <button
                                                                type="button"
                                                                onClick={() => {
                                                                    setForumReplyingTo({ id: reply.id, name: replyAuthor });
                                                                    setReplyContent(`@${replyAuthor} `);
                                                                    forumCommentInputRef.current?.focus();
                                                                }}
                                                                className="text-[10px] font-bold text-amber-500 dark:text-gold uppercase tracking-wider hover:opacity-85 transition-opacity"
                                                            >
                                                                Reply
                                                            </button>
                                                            {likes > 0 && (
                                                                <span className="text-[10px] text-gray-400 dark:text-gray-500 font-medium">{likes} {likes === 1 ? 'like' : 'likes'}</span>
                                                            )}
                                                        </div>
                                                    </div>
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            ...
                                                            const isLiked = !localLikedForumComments[replyId];
                                                            const updatedLiked = { ...localLikedForumComments, [replyId]: isLiked };
                                                            setLocalLikedForumComments(updatedLiked);
                                                            localStorage.setItem('liked_forum_comments', JSON.stringify(updatedLiked));
                                                        }}
                                                        className={`shrink-0 p-1.5 rounded-full hover:bg-gray-100 dark:hover:bg-white/10 transition-colors ${isReplyLiked ? 'text-red-500' : 'text-gray-400 hover:text-red-500'}`}
                                                    >
                                                        <Heart className={`w-3.5 h-3.5 ${isReplyLiked ? 'fill-red-500 text-red-500' : ''}`} />
                                                    </button>
                                                </div>
                                                ...
                                                {nestedReplies.length > 0 && (
                                                    <div className="ml-11 mt-3 space-y-3">
                                                        <button
                                                            type="button"
                                                            onClick={() => {
                                                                setExpandedForumReplies(prev => ({ ...prev, [reply.id]: !prev[reply.id] }));
                                                            }}
                                                            className="flex items-center gap-1 text-[10px] font-bold text-gray-400 dark:text-gray-500 hover:text-gray-900 dark:hover:text-white transition-colors mb-1"
                                                        >
                                                            <span className="w-3 h-[1px] bg-gray-300 dark:bg-white/20 inline-block mr-1"></span>
                                                            {expandedForumReplies[reply.id] ? `Hide replies` : `View replies (${nestedReplies.length})`}
                                                        </button>

                                                        {expandedForumReplies[reply.id] && (
                                                            <div className="space-y-3">
                                                                {nestedReplies.map((nested: any) => {
                                                                    const nestedAuthor = nested.author?.displayName || nested.author?.display_name || 'Anonymous';
                                                                    const nestedAvatar = nested.author?.avatarUrl || nested.author?.avatar_url;
                                                                    const isNestedLiked = !!localLikedForumComments[nested.id];
                                                                    const nestedLikes = isNestedLiked ? 1 : 0;

                                                                    return (
                                                                        <div key={nested.id} className="flex gap-2.5 text-left items-start relative bg-gray-50 dark:bg-white/[0.02] p-2 rounded-lg">
                                                                            <Avatar name={nestedAuthor} src={nestedAvatar} size="xs" className="!w-6 !h-6 shrink-0 !ring-1 !ring-gray-200 dark:!ring-white/10" />
                                                                            <div className="flex-1 min-w-0">
                                                                                <div className="flex items-baseline gap-1.5">
                                                                                    <span className="text-gray-900 dark:text-white text-[11px] font-bold truncate">{nestedAuthor}</span>
                                                                                    <span className="text-gray-400 dark:text-gray-500 text-[9px] shrink-0">{formatTimeAgo(nested.createdAt || nested.created_at)}</span>
                                                                                </div>
                                                                                <p className="text-gray-700 dark:text-gray-200 text-xs leading-[1.4] mt-0.5 whitespace-pre-wrap break-words">{nested.content}</p>
                                                                                ...
                                                                                <div className="flex items-center gap-3 mt-1">
                                                                                    <button
                                                                                        type="button"
                                                                                        onClick={() => {
                                                                                            setForumReplyingTo({ id: reply.id, name: nestedAuthor });
                                                                                            setReplyContent(`@${nestedAuthor} `);
                                                                                            forumCommentInputRef.current?.focus();
                                                                                        }}
                                                                                        className="text-[9px] font-bold text-amber-500 dark:text-gold uppercase tracking-wider hover:opacity-85 transition-opacity"
                                                                                    >
                                                                                        Reply
                                                                                    </button>
                                                                                    {nestedLikes > 0 && (
                                                                                        <span className="text-[9px] text-gray-400 dark:text-gray-500 font-medium">{nestedLikes} {nestedLikes === 1 ? 'like' : 'likes'}</span>
                                                                                    )}
                                                                                </div>
                                                                            </div>
                                                                            ...
                                                                            <button
                                                                                type="button"
                                                                                onClick={() => {
                                                                                    const replyId = nested.id;
                                                                                    const isLiked = !localLikedForumComments[replyId];
                                                                                    const updatedLiked = { ...localLikedForumComments, [replyId]: isLiked };
                                                                                    setLocalLikedForumComments(updatedLiked);
                                                                                    localStorage.setItem('liked_forum_comments', JSON.stringify(updatedLiked));
                                                                                }}
                                                                                className={`shrink-0 p-1 rounded-full hover:bg-gray-100 dark:hover:bg-white/10 transition-colors ${isNestedLiked ? 'text-red-500' : 'text-gray-400 hover:text-red-500'}`}
                                                                            >
                                                                                <Heart className={`w-3 h-3 ${isNestedLiked ? 'fill-red-500 text-red-500' : ''}`} />
                                                                            </button>
                                                                        </div>
                                                                    );
                                                                })}
                            ...
                            {/* Replying indicator */}
                            {forumReplyingTo && (
                                <div className="flex items-center justify-between bg-amber-500/10 dark:bg-gold/10 border border-amber-500/20 dark:border-gold/20 px-4 py-2 mx-4 rounded-xl">
                                    <span className="text-xs text-amber-600 dark:text-gold font-medium">
                                        Replying to @{forumReplyingTo.name}
                                    </span>
                                    <button
                                        type="button"
                                        onClick={() => setForumReplyingTo(null)}
                            ...
                            {/* Quick Emoji Bar */}
                            <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none px-4 py-1.5 border-t border-gray-100 dark:border-white/5">
                                {['❤️', '🙌', '🔥', '😮', '👏', '😍', '💡', '🎨'].map(emoji => (
                                    <button
                                        key={emoji}
                                        type="button"
                                        ...
                                        className="text-sm p-1 rounded hover:bg-gray-100 dark:hover:bg-white/10 transition-colors"
                                    >
                                        {emoji}
                                    </button>
                                ))}
                            ...
                            >
                                ...
                                <input 
                                    ...
                                    placeholder={forumReplyingTo ? "Write a reply..." : "Add a reply..."} 
