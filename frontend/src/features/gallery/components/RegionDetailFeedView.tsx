...
import { RegionDetail, classifyPlace, getRealPlaceCoverImage } from '../data/citiesRegistry';
            ...
            {isLoading ? (
                ...
                <div className="space-y-4">
                    {filteredPlaces.map((place) => {
                        ...
                        // Extract cover image using high-fidelity real place name resolver
                        let displayImage = getRealPlaceCoverImage(place.name, matchedType, place.cover_image_url);
                        if (!displayImage && place.photos && place.photos.length > 0) {
                            const firstPhoto = place.photos[0];
                            if (typeof firstPhoto === 'string' && firstPhoto.startsWith('http')) {
                                displayImage = getRealPlaceCoverImage(place.name, matchedType, firstPhoto);
                            }
                        }
                        if (!displayImage) {
                            displayImage = getRealPlaceCoverImage(place.name, matchedType);
                        }
